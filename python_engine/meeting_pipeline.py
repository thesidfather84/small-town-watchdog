"""
Meeting and agenda pipeline.
Ingests sources with source_category in ('agenda-page', 'minutes-page').
Creates civic_items with item_type='meeting'.
"""
...
_MEETING_CATEGORIES = {"agenda-page", "minutes-page"}


def run(state: Optional[str] = None) -> PipelineRun:
    """
    Import meeting agendas and minutes.
    Filters source_registry to agenda/minutes categories.
    """
    stats = PipelineRun()
    all_sources = db.load_active_sources(state=state)
    meeting_sources = [
        s for s in all_sources
        if s.get("source_category") in _MEETING_CATEGORIES
    ]

    log.info("Meeting pipeline: processing %d sources…", len(meeting_sources))

    for src in meeting_sources:
        ...
        fetched = fetch(url)
        if fetched is None:
            stats.errors.append(f"Meeting fetch failed: {url}")
            continue
        ...
        category = src.get("source_category", "")
        item_type = "meeting"
        title = parsed.title or (
            f"{'Agenda' if 'agenda' in category else 'Minutes'} — {src.get('entity_name', 'Unknown')}"
        )
        ...
        try:
            ...
            if not created:
                ...
                log.debug("Meeting duplicate skipped: %s", title)
            else:
                stats.items_created += 1
                if admin_status == "approved":
                    stats.items_auto_approved += 1
                    log.info("Meeting auto-approved %d: %s", new_id, title)
                else:
                    stats.items_pending += 1
                    log.info("Meeting item %d pending review: %s", new_id, title)
        except Exception as e:
